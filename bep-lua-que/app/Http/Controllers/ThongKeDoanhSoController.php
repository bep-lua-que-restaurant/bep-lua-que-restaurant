    ...
    public function index(Request $request)
    {
        ...
        if ($fromDate && $toDate) {
            ...
            if ($filterType === 'day') {
                ...
                $rawData = HoaDon::selectRaw('DATE(created_at) as date, SUM(tong_tien) as revenue')
                ...
                $rawData = HoaDon::selectRaw('DATE_FORMAT(created_at, "%Y-%m") as month, SUM(tong_tien) as revenue')
                    ->whereBetween('created_at', [$from, $to])
                    ->groupBy('month')
                    ->orderBy('month')
                    ->pluck('revenue', 'month')
                ...
                $rawData = HoaDon::selectRaw('YEAR(created_at) as year, SUM(tong_tien) as revenue')
                    ->whereBetween('created_at', [$from, $to])
                    ->groupBy('year')
                    ->orderBy('year')
                    ->pluck('revenue', 'year')
