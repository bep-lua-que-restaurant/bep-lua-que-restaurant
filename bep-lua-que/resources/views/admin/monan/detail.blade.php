    ...
    <div class="container my-4">
        ...
        {{-- <div class="col-12 mb-4">
            ...
            @if ($monAn->congThuc->isEmpty())
                <div class="border p-2 rounded bg-light">Chưa có công thức</div>
            @else
                <table class="table table-bordered table-striped mt-2">
                    ...
                    <tbody>
                        @foreach ($monAn->congThuc as $index => $ct)
                            <tr>
                                <td>{{ $index + 1 }}</td>
                                <td>{{ $ct->nguyenLieu->ten_nguyen_lieu?? 'Đã xóa' }}</td>
                                <td>{{ $ct->so_luong }}</td>
                                <td>{{ $ct->don_vi ?? 'Không có' }}</td>
                            </tr>
                        @endforeach
                    </tbody>
                </table>
