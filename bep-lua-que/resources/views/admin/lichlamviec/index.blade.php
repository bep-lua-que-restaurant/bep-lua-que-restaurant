...
#toggleSearchDropdown:hover {
    background-color: #3c9641;
    color: white;
}
.filter-wrapper {
    margin-top: 10px; /* Khoảng cách từ textbox xuống button */
}

.search-box {
    margin-top: -10px; /* Dịch lên trên */
}
    ...
    <script>
        ...
        document.querySelectorAll(".add-schedule").forEach((cell) => {
            cell.addEventListener("click", function() {
                let shiftName = this.getAttribute("data-shift");
                let dayIndex = Array.from(this.parentElement.children).indexOf(this);
                let dayNames = ["Thứ 2", "Thứ 3", "Thứ 4", "Thứ 5", "Thứ 6", "Thứ 7", "Chủ nhật"];
                document.getElementById("selectedDay").textContent = dayNames[dayIndex - 1];
                document.getElementById("shiftTitle").textContent = shiftName;
