...
@section('content')
    <style>
        .table-primary th {
            ...
            /* Chuyển chữ thành in hoa (tùy chọn) */
        }
    </style>
    <div class="container mt-4">
        <div class="card shadow-lg rounded-4 border-0">
            ...
            <div class="card-body bg-light-subtle">
                <div class="row mb-3">
                    <div class="col-md-4"><strong>Mã phiếu:</strong> <span
                            class="text-primary">{{ $phieuXuatKho->ma_phieu }}</span></div>
                    <div class="col-md-4"><strong>Ngày xuất:</strong> <span
                            class="text-primary">{{ \Carbon\Carbon::parse($phieuXuatKho->ngay_xuat)->format('d/m/Y') }}</span>
                    </div>
                    <div class="col-md-4">
                ...
                <div class="row mb-3">
                    <div class="col-md-4"><strong>Nhân viên:</strong> <span
                            class="text-primary">{{ $phieuXuatKho->nhanVien->ho_ten ?? '---' }}</span></div>
                    @if ($phieuXuatKho->loai_phieu === 'xuat_tra_hang')
                        <div class="col-md-4"><strong>Nhà cung cấp:</strong> <span
                                class="text-primary">{{ $phieuXuatKho->nhaCungCap->ten_nha_cung_cap ?? '---' }}</span></div>
                    @else
                        <div class="col-md-4"><strong>Người nhận:</strong> <span
                                class="text-primary">{{ $phieuXuatKho->nguoi_nhan ?? '---' }}</span></div>
                    @endif
                    <div class="col-md-4"><strong>Ghi chú:</strong> <span
                            class="text-secondary fst-italic">{{ $phieuXuatKho->ghi_chu ?? '---' }}</span></div>
                </div>
                ...
                <div class="table-responsive">
                        ...
                        <tbody class="table-light">
                            @foreach ($chiTietPhieuXuatKhos as $index => $item)
                                <tr>
                                    ...
                                    <td class="text-secondary">{{ $item->nguyenLieu->loaiNguyenLieu->ten_loai ?? '---' }}
                                    </td>
                                    <td>{{ $item->don_vi_xuat }}</td>
                                    <td>{{ $item->so_luong }}</td>
                                    <td>{{ number_format($item->don_gia, 0, ',', '.') }} đ</td>
                                    <td class="text-end text-danger fw-bold">
                                        {{ number_format((float) $item->so_luong * (float) str_replace('.', '', $item->don_gia), 0, ',', '.') }}
                                        đ
                                    </td>
                        ...
                        <tfoot>
                            <tr class="table-secondary fw-bold">
                                ...
                                <td class="text-end text-primary">
                                    {{ number_format($chiTietPhieuXuatKhos->sum(fn($i) => $i->so_luong * $i->don_gia), 0, ',', '.') }}
                                    đ
                                </td>
                                <td></td>
                            </tr>
                    ...
                    @if ($phieuXuatKho->trang_thai === 'cho_duyet')
                        <div class="d-flex justify-content-end gap-2 mt-4">
                            {{-- Nút Duyệt phiếu --}}
                            <form action="{{ route('phieu-xuat-kho.duyet', $phieuXuatKho->id) }}" method="POST"
                            ...
                            {{-- Nút Hủy phiếu --}}
                            <form action="{{ route('phieu-xuat-kho.huy', $phieuXuatKho->id) }}" method="POST"
