    ...
    <tr data-toggle="collapse" data-target="#detail{{ $index }}" class="clickable-row">
        ...
        <td>
            <div class="d-flex align-items-center">
                <a href="{{ route('com-bo.show', $item->id) }}" class="btn btn-info btn-sm p-2 m-2">
                    <i class="fa fa-eye"></i>
                </a>
                <a href="{{ route('com-bo.edit', $item->id) }}" class="btn btn-warning btn-sm p-2 m-2">
                    <i class="fa fa-edit "></i>
                </a>
                @if ($item->deleted_at)
                    <form action="{{ route('com-bo.restore', $item->id) }}" method="POST" style="display:inline;">
                        @csrf
                ...
                @else
                    <form action="{{ route('com-bo.destroy', $item->id) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" onclick="return confirm('Bạn muốn ngừng combo này chứ?')"
